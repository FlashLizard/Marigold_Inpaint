...
# 模拟命令行参数
sys.argv = [
    "run_controlnet.py",  # 模拟脚本名称
    # "--checkpoint", "prs-eth/marigold-depth-v1-0",
    "--checkpoint", "checkpoint/marigold-depth-v1-0",
    "--denoise_steps", "50",
    "--ensemble_size", "10",
    "--input_rgb_dir", "input/img",
    "--output_dir", "output"
]
...
if "__main__" == __name__:
    ...
    with torch.no_grad():
        ...
        for rgb_path in tqdm(rgb_filename_list, desc="inpaint depth", leave=True):
            # Read input image
            input_image = Image.open(rgb_path)
            basename = os.path.basename(rgb_path)
            base_dir = os.path.dirname(os.path.dirname(rgb_path))
            input_depth = np.load(os.path.join(base_dir, 'depth', basename[:-4]+'.npy'))
            input_mask = Image.open(os.path.join(base_dir, 'mask', basename[:-4]+'.jpg'))
